...
class UserRepository {
  ...
  async save(userData: Pick<IUser,"userName"|"email"|"password"|"phoneNumber">) {
    try {
...
async get(searchQuery?: Partial<IUser>) {
  try {
    ...
    if (searchQuery?.userName?.firstName) {
      query["userName.firstName"] = { $regex: searchQuery.userName.firstName, $options: "i" }; // case-insensitive
    }
    if (searchQuery?.userName?.lastName) {
      query["userName.lastName"] = { $regex: searchQuery.userName.lastName, $options: "i" };
    }
  ...
  async update(id: string, data: Pick<IUser,"userName"|"email"|"password"|"phoneNumber">) {
    try {
